...
给每一个电影的内容一个数值，然后对一个用户对这个电影进行线性回归，求他对这个电影的打分。

在我们的例子中，我们可以假设每部电影都有两个特征，如$𝑥^{(1)}$代表电影的浪漫程度，$𝑥^{(2)}$代表电影的动作程度。

![](https://raw.githubusercontent.com/mm0806son/Images/main/202111031554744.png)

$𝜃^{(𝑗)}$用户 $𝑗$ 的参数向量：用户对不用类型电影的偏好；
$𝑥^{(i)}$电影 $𝑖$ 的特征向量：电影更属于哪一类型。
